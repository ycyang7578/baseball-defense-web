...
- `src/optimization.py` — 站位最佳化：`optimize_positions(batter_id, on_1b, on_2b, on_3b, outs, ..., home_team=None)`
  ...
  - **模型參數口徑（2026-07-13 定案：統一 OF）**：`optimize_positions` 與顯示層
    （API eval、precompute_model_oaa）一律用 `models/{year}/OF/` 統一參數，三位置
    共用。⚠️ `models/2025/` 底下另存在分位置目錄（LF/CF/RF/，舊模型時代遺留，
    repo 初始匯入 a86b085 一起帶進來），`_resolve_model_dir` 會**偏好**分位置目錄
    ——曾使優化器（分位置）與顯示層（統一 OF）在 2025 用兩套曲面，兩組站位的
    優劣排序可以相反（實測 Judge 得點圈壘況差 ±1.34 分、且出現 CF/RF 假換位）。
    修正：optimize_positions 明確載 "OF"。若日後新增分位置模型要先想清楚這裡
...
- `scripts/validate/validate_if_positioning.py` — 階段 5 跨年驗證 → `models/if_gb/validation_2025.json`
  ...
  - **2026-07-13 現行結果（最終模型：2023–24、無 stand_R、貝葉斯群體層優化器）**：
    跨年增益平均 **+0.0156/GB**（每 450 GB ≈ +7.0 出局）、同年上限 +0.0245、
    保留率 **63.6%**、95.3% 打者正增益；t=20.75、p=7.6e-53；
    左打增益高於右打（L +0.0175 / R +0.0146，方向呼應 shift 文獻的左打效應）
  - （歷史對照 2026-07-07，舊 GLM：+0.0155/GB、保留率 64.0%、96.2% 正增益、
    t=21.50——換最終模型後幾乎完全重現）
...
- **階段B：一壘有人雙殺情境的站位優化（2026-07-13 開工，研究階段、未接 web）**：
  ...
  - **B5 跨年驗證＋增益分解（2026-07-13 完成，n=212、一壘有人 0 出局、皆評
    ...
    | 聯盟 → p1 解（適應釘死 1B 的幾何，不懂雙殺） | **+0.0704**（占總增益 97%） |
    | p1 解 → DP 解（雙殺感知計價的淨貢獻） | **+0.0019**（450GB≈+0.9 分、59.4% 正，邊際） |
    **解讀**：與階段A 一致的敘事——計價精緻化（run-value、DP 感知）對站位選擇
    是二階；一階的是**幾何**。但「壘況的幾何約束」（1B hold runner 改變覆蓋
...
- **特徵形式消融（2026-07-12）**：`scripts/experiments/exp_if_drop_features.py`（訓練 2023–24、
  ...
  同步改，等價測試 12/12 過（GBM 的 stand_R 保留）。
  ②launch_speed 不可移除：−0.0132 AUC（處理難度＋慢滾球減速誤差海綿）。
  ③「全特徵 spline」AUC +0.015（兩驗證年一致）**但已否決**：增益幾乎全來自
  throw_dist spline（單獨 +0.012），其偏效應呈守位身分波浪（throw_dist 分箱
  幾乎=守位標籤：0-50 呎=1B、50-95=2B、95-115=2B/SS 交界混合區出局率 0.546、
  115+=SS/3B），105 呎谷與 130 呎假凸起會誤導優化器——throw_dist 是攔截幾何
  （spray×深度）的確定函數，給彈性形狀等於從後門重新編碼被排除的 raw spray
